...
const CourseDetailPage = () => {
  ...
  const handleChange=async(e)=>{
    try{
      const response = await fetch(`http://localhost:8000/api/enrolledcourse/check/${activeUser._id}/${courseId}`,{
        method:'POST',
        credentials:'include'
      })
      ...
      if(response.ok){
        ...
        setalert({
          variant:"destructive",
          title:"Failure!!",
          description:data.message || "Failure"
        })
      }
    }
    catch(error){
  ...
  useEffect(() => {
    try {
      ...
      console.log("Error while fetching single course", error);
    }
    finally {
      setloading(false);
    }
  ...
  return (
    <>
      ...
      {
        loading ? (
          <h1>Single Course is loading</h1>
        ) : (
          <>
            {singlecourse ? (
              <>
                <div className={`bg-gray-50 dark:bg-gray-900 min-h-screen transition-colors duration-500`}>
                  ...
                  {alert && (
                    <Alert variant={alert.variant} className="m-4">
                          ...
                          <div className="flex flex-col md:flex-row gap-6">
                            ...
                            <div className="md:w-2/3">
                              ...
                              <div className="flex items-center justify-between border-t pt-4 border-gray-200 dark:border-gray-700">
                                ...
                                <button onClick={handleChange} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded-lg text-lg transition duration-300 shadow-md transform hover:scale-[1.02] focus:ring-4 focus:ring-indigo-300">
                                  Enroll Now
                                </button>
                              </div>
                        ...
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl sticky top-8">
                          ...
                          <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700 text-center">
                            ...
                            <button className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded-lg text-lg transition duration-300">
                              <Tooltip>
                                <TooltipTrigger>I want this Course!!</TooltipTrigger>
                                <TooltipContent>
                                  <p>Go to Above Enroll Now Button</p>
                                </TooltipContent>
                              </Tooltip>
                            </button>
                          </div>
              ...
              <h1>Single Course exist ni krta hai.</h1>
            )
            }
          </>
        )
